    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: "16px", marginTop: "12px" }}>
        ...
        <div style={card}>
          ...
          <table style={{ width: "100%", color: "#0F172A" }}>
            <thead><tr><th>ID</th><th>Name</th><th>Phone</th><th>Gov ID</th><th>House</th><th>Start</th><th></th></tr></thead>
            <tbody>
              {active.map(t => (
                <tr key={`${t.id}-active`}>
                  <td>{t.id}</td><td>{t.full_name}</td><td>{t.phone}</td><td>{t.gov_id}</td><td>{t.house_number}</td><td>{t.start_date}</td>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"16px", marginTop:"16px" }}>
        <div style={card}>
          ...
          <table style={{ width: "100%", color: "#0F172A" }}>
            <thead><tr><th>ID</th><th>Name</th><th>Phone</th><th>Gov ID</th><th>House</th><th>End</th></tr></thead>
            <tbody>
              {former.map(t => <tr key={`${t.id}-former`}><td>{t.id}</td><td>{t.full_name}</td><td>{t.phone}</td><td>{t.gov_id}</td><td>{t.house_number}</td><td>{t.end_date}</td></tr>)}
            </tbody>
          </table>
        </div>
        <div style={card}>
          ...
          <table style={{ width: "100%", color: "#0F172A" }}>
            <thead><tr><th>ID</th><th>Name</th><th>Phone</th><th>Gov ID</th><th>Email</th></tr></thead>
            <tbody>
              {unassigned.map(t => <tr key={`${t.id}-unassigned`}><td>{t.id}</td><td>{t.full_name}</td><td>{t.phone}</td><td>{t.gov_id}</td><td>{t.email || "-"}</td></tr>)}
            </tbody>
          </table>
        </div>
      </div>
