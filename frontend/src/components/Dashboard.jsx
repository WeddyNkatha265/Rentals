export default function Dashboard({ stats, loading, error, onRefresh }) {
  ...
  const bar = (val) => ({ width: 24, height: Math.max(4, Math.min(120, (val / Math.max(1, maxVal())) * 120)), background: "linear-gradient(180deg, #1D4ED8, #F97316)", borderRadius: 6 });
  ...
  return (
    <div>
      ...
      <button
        onClick={onRefresh}
        style={{ marginTop: 12, background: "linear-gradient(90deg, #1D4ED8, #F97316)", color: "white", padding: "8px 12px", borderRadius: 10, border: "none", boxShadow: "0 4px 12px rgba(10,37,64,0.2)" }}
      >
        Refresh
      </button>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px", marginTop: "16px" }}>
        <div style={card}>
          ...
          <table style={{ width: "100%", marginTop: 8, color: "#0F172A" }}>
            ...
            <tbody>
              {(stats.top_houses || []).map((h, i) => (<tr key={i}><td>{h.house_number}</td><td>KES {h.received}</td></tr>))}
            </tbody>
          </table>
        </div>
        <div style={card}>
          ...
          <table style={{ width: "100%", marginTop: 8, color: "#0F172A" }}>
            <thead><tr><th>House</th><th>Tenant</th><th>Amount</th><th>Method</th><th>Date/time</th></tr></thead>
            <tbody>
              {(stats.recent_payments || []).map((p, i) => (
                <tr key={i}><td>{p.house_number}</td><td>{p.tenant_name}</td><td>KES {p.amount}</td><td>{p.method}</td><td>{p.paid_at}</td></tr>
              ))}
            </tbody>
          </table>
        </div>
